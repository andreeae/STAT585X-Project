...
laply(d,function(x) names(x) <- unique(allcrops))  ## doent do it automatically
...
for (i in 1:cropmaxno)
    if (length(grep(unique(allcrops)[i],names(d[[1]])))<1) {
      d[[1]] <- c(d[[1]], NA)
      names(d[[1]])[i] <- unique(allcrops)[i]
    } 

for (i in 1:cropmaxno)
    if (length(grep(unique(allcrops)[i],names(d[[2]])))<1) {
      d[[2]] <- c(d[[2]], NA)
      names(d[[2]])[i] <- unique(allcrops)[i]
    } 

for (i in 1:cropmaxno)
    if (length(grep(unique(allcrops)[i],names(d[[3]])))<1) {
      d[[3]] <- c(d[[3]], NA)
      names(d[[3]])[i] <- unique(allcrops)[i]
    } 

for (i in 1:cropmaxno)
    if (length(grep(unique(allcrops)[i],names(d[[4]])))<1) {
      d[[4]] <- c(d[[4]], NA)
      names(d[[4]])[i] <- unique(allcrops)[i]
    } 
for (i in 1:cropmaxno)
    if (length(grep(unique(allcrops)[i],names(d[[5]])))<1) {
      d[[5]] <- c(d[[5]], NA)
      names(d[[5]])[i] <- unique(allcrops)[i]
    } 

cbind(as.data.frame(d[[1]]),as.data.frame(d[[2]]),,as.data.frame(d[[3]]),as.data.frame(d[[4]]),as.data.frame(d[[5]]))
...
\hline
min &  \Sexpr{d[1,1]} & \Sexpr{d[1,2]}& \Sexpr{d[1,3]}&\Sexpr{d[1,4]} & \Sexpr{d[1,5]}& \Sexpr{d[1,6]}& \Sexpr{d[1,7]}\\ 
1stq &  \Sexpr{d[2,1]}  & \Sexpr{d[2,2]} & \Sexpr{d[2,3]} & \Sexpr{d[2,4]} & \Sexpr{d[2,5]} & \Sexpr{d[2,6]} & \Sexpr{d[2,7]}\\ 
median & \Sexpr{d[3,1]}  & \Sexpr{d[3,2]} & \Sexpr{d[3,3]} & \Sexpr{d[3,4]}& \Sexpr{d[3,5]} & \Sexpr{d[3,6]} & \Sexpr{d[3,7]}\\ 
mean   & \Sexpr{d[4,1]}  & \Sexpr{d[4,2]} & \Sexpr{d[4,3]} & \Sexpr{d[4,4]} & \Sexpr{d[4,5]} & \Sexpr{d[4,6]}& \Sexpr{d[4,7]}\\ 
3rdq  &  \Sexpr{d[5,1]}  & \Sexpr{d[5,2]} & \Sexpr{d[5,3]} & \Sexpr{d[5,4]} & \Sexpr{d[5,5]} & \Sexpr{d[5,6]}& \Sexpr{d[5,7]}\\ 
max   &   \Sexpr{d[6,1]}  & \Sexpr{d[6,2]} & \Sexpr{d[6,3]} & \Sexpr{d[6,4]}& \Sexpr{d[6,5]} & \Sexpr{d[6,6]}& \Sexpr{d[6,7]}\\ 
\hline
\hline
sd &  \Sexpr{d[7,1]} $ & \Sexpr{d[7,2]} & \Sexpr{d[7,3]} & \Sexpr{d[7,4]} & \Sexpr{d[7,5]} & \Sexpr{d[7,6]}& \Sexpr{d[7,7]}\\ 
se &  \Sexpr{d[8,1]} $ & \Sexpr{d[8,2]} & \Sexpr{d[8,3]} & \Sexpr{d[8,4]} & \Sexpr{d[8,5]} & \Sexpr{d[8,6]}& \Sexpr{d[8,7]}\\ 
\hline
\end{tabular}
