...
\title{STAT 585X - Final Project - Report \\ Changes in cultivated cropland land in Iowa}
...
Do land characteristics changes over time match the Conservation Effects Assessement Project (CEAP) survey design and data collection?
...
For CSSM analysis, data from the Soil Survey is considered, collected over the 2003 to 2006 time period. Hence, the survey frame, consisting of NRI cultivated cropland points, is set in 2003. The information for these eligible points is collected in the following years, 2004-2007.
...
#first step is to set the working directory to the folder where the data is saved
...
The next step is to utilize the tools constructed in this project to analyze changes in the real CEAP data. We would also like to investigate the CDL data as opossible source of covariates in CEAP small area models.
